 ...
 * Importa comercios creados entre 2025-10-01 y 2026-06-01.
 * NO borra comercios existentes — solo inserta los que no existen (por merchant_id).
 * 
 * Ejecutar: npx ts-node src/database/import_commerces.ts
 * O dentro del contenedor: node dist/database/import_commerces.js
 */
...
async function main() {
  ...
  const [commerces] = await mysqlConn.execute(
    `SELECT id, name, slug, rut, country, enabled, created_at, updated_at
     FROM commerce
  ...
  // 2. Obtener merchant_ids ya existentes en el CRM (para no duplicar)
  ...
  // También verificar por nombre para evitar duplicados
  ...
  const toImport = commerces.filter((c: any) => {
    // No importar si ya existe por merchant_id
    if (existingIds.has(String(c.id))) return false;
    // No importar si ya existe por nombre exacto
  ...
  for (const c of toImport as any[]) {
    try {
      await pgPool.query(
        ...
        [
          c.name || 'Sin nombre',           // legal_name
          c.name || null,                    // trade_name
          c.rut || null,                     // tax_id
          c.country || null,                 // country
          'lead',                            // status (nuevo, sin procesar)
          'diamond',                         // risk_level (default)
          String(c.id),                      // merchant_id (ID de producción)
          c.created_at || new Date(),        // created_at
          c.updated_at || new Date(),        // updated_at
          c.updated_at || c.created_at || new Date(), // last_activity_at
        ]
      );
      imported++;
      if (imported % 10 === 0) {
